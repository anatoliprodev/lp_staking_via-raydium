...
pub fn process_create_state(ctx: Context<CreateFaucetState>, state_nonce:u8, mint_usdc_usdr_lp_nonce:u8, mint_eth_sol_lp_nonce:u8, mint_atlas_ray_lp_nonce:u8, mint_samo_ray_lp_nonce:u8) -> ProgramResult {
    ctx.accounts.faucet_state.super_owner = ctx.accounts.super_owner.key();
    ctx.accounts.faucet_state.mint_usdc_usdr_lp = ctx.accounts.mint_usdc_usdr_lp.key();
